...
\section{Penelitian Terdahulu}\label{}
...
\section{Gim}\label{}
Istilah gim merupakan padanan bahasa Indonesia dari kata \textit{game} yang berarti “permainan” dalam konteks digital maupun non-digital \citep{danny2021valorant}. Dalam KBBI, gim didefinisikan sebagai “permainan", terutama yang menggunakan komputer, konsol, atau gawai \citep{KBBI2026}. Hal ini menandakan bahwa istilah gim telah diakui secara linguistik dan sesuai dengan upaya pembakuan bahasa Indonesia terhadap terminologi teknologi dan digital modern.
...
Dalam desain ekonomi gim, sistem \textit{reward} merupakan komponen teknis yang berfungsi sebagai sumber (\textit{source}) . Sistem ini bertanggung jawab dalam memasukkan atau menambahkan sumber daya virtual ke dalam ekosistem pemain \citep{RanandehMirzaBabaei2023}. Implementasinya bervariasi mulai dari hadiah atas hasil penyelesaian misi, item acak berbeasis probabilitas, hingga mekanisme crafting yang mengubah input menjadi output. 
...
Balancing dalam game merujuk pada proses penyesuaian mekanik, aturan, serta atribut elemen permainan agar seluruh pilihan yang tersedia bagi pemain berada dalam kondisi yang adil dan layak digunakan. Rollings dan Adams menyatakan bahwa sebuah game dikatakan seimbang apabila keberhasilan pemain terutama ditentukan oleh keterampilan, bukan oleh keberuntungan semata atau dominasi satu strategi tertentu \cite{RollingsAdams2003}. Keseimbangan ini bertujuan mencegah munculnya \textit{dominant strategy} yang dapat menghilangkan variasi dan makna dalam pengambilan keputusan.

Sirlin menekankan bahwa balancing tidak berarti seluruh opsi harus memiliki kekuatan yang sama, melainkan setiap opsi harus memiliki kelebihan dan kelemahan yang jelas sehingga menghasilkan pilihan yang bermakna dan adil, terutama pada permainan kompetitif \cite{Sirlin2016}. Opsi yang terlalu kuat akan mendominasi, sedangkan opsi yang terlalu lemah menjadi tidak relevan dan mengurangi kedalaman permainan.
...
\label{alg:ea-reward}
\begin{algorithmic}[1]
\Require Populasi awal $P_0$, fungsi \textit{fitness} $f(\cdot)$, probabilitas \textit{crossover} $p_c$, probabilitas mutasi $p_m$, kondisi terminasi
\Ensure Solusi terbaik $x^*$
